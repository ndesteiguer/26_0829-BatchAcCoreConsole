...
> **Scope authority:** [PRODUCT_SCOPE.md](PRODUCT_SCOPE.md) controls product scope. [EXECUTION_CONTRACT.md](EXECUTION_CONTRACT.md) defines the target routine-definition and lifecycle details. This plan replaces prototype-specific LISP signature, CSV, and compatibility assumptions with the target LISP-or-SCR execution model.
...
The core accepts an execution definition rather than inferring a routine contract from source or filename. It distinguishes the selected execution type and records only the information needed to invoke it: an explicit AutoLISP entry point or standalone SCR path, an optional shared input-file path for a LISP routine, and its declared output mode.
...
- Keep routine metadata lightweight and external. It declares execution type, entry point, input mode (`None` or shared input file), output mode (`None`, `OnePerFile`, or `MultiLinePerFile`), and any combination metadata. The application does not parse source to discover dependencies, parse the input file, author routine code, or interpret LISP/SCR behavior.

## Execution design

The core compiles the selected execution definition into the appropriate Core Console launcher flow. An AutoLISP run loads the selected file and invokes the explicitly configured entry point, passing the declared shared input-file path when applicable. A standalone SCR run executes the selected script directly. Both modes use the same worker isolation, process monitoring, timeout, logging, cancellation, and application-owned status reporting.
...
1. **Profile validation:** execution type, required files and paths, explicit AutoLISP entry point when applicable, declared shared input-file existence/readability, input-method choice, worker/timeout range, and work/results distinction.
...
Preflight does not validate LISP `defun` signatures, parse SCR commands or shared input-file contents, resolve routine dependencies, or determine whether a routine is semantically correct. The run command repeats mandatory filesystem and execution-setting validation to prevent bypass through the CLI or stale GUI state.
...
- Unit tests for profile validation, execution-type selection, declared shared input-file validation, drawing discovery, launcher generation, output declaration validation, and output-combination compatibility.
...
1. Define the execution-profile, routine-metadata, optional shared input-file, result-artifact, and output-declaration contracts.
